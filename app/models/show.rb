...
#highest_rating: this method should return the highest value in the ratings column. hint: if there is a #minimum Active Record method, might there be a #maximum method?
    ...
    #popular_self: returns an array of all of the self that have a rating greater than 5. hint: use the #where Active Record method.
    def self.popular_shows
        self.where("rating > 5")
    end
    #self_by_alphabetical_order: returns an array of all of the self sorted by alphabetical order according to their names. hint: use the #order Active Record method.
